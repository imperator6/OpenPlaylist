...
- Vote Sort: An admin-only toggle ("Vote sort") in the nav bar enables automatic queue re-sorting by net votes (upvotes minus downvotes) after each vote. Only the next 10 tracks after the currently-playing track are sorted (windowed sorting); the rest of the queue stays in its original order. The UI uses FLIP animations so cards smoothly slide to their new positions after a vote changes the order.
